...
const Sync = async () => {
    ...
    const fileHref = await Plugins.picker.single(
        "请选择要同步至本地的备份",
        _list,
        [_list[0].value],
    );

    const {update, destroy, success, error} = Plugins.message.info(
        "获取备份文件...",
        60 * 60 * 1000,
    );
    ...
    if (kernelApiStore.running) {
        // 自己修改
        console.log(
            `kernelApiStore.running...，但是，手动注释了原作者 await kernelApiStore.stopCore() 方法，直接 await Plugins.WindowReloadApp()...`,
        );
...
const Backup = async () => {
    ...
    const l3 = rulesetsStore.rulesets
        .map((v) => v.path)
        .filter(
            (v) => v.startsWith("data") && (v.endsWith("yaml") || v.endsWith("json")),
        );
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        Plugins.message.info(id, `正在创建备份...[ ${file} ]`);
        // console.log(id, `正在创建备份...[ ${file} ]`);
        try {
    ...
    try {
        ...
        const dav = new WebDAV(Plugin.Address, Plugin.Username, Plugin.Password);
        await dav.put(
            Plugin.DataPath + "/" + backupFilename,
...
const getBackupFilename = async () => {
    const defaultFilename =
        ...
        Plugins.APP_VERSION +
        "_" +
        Plugins.formatDate(Date.now(), "YYYYMMDD-HHmmss") +
        "_" +
        "core-" +
        ((await getKernelVersion()) + `-自用`);

    const input =
        (await Plugins.prompt(Plugin.name, defaultFilename)) || defaultFilename;
    if (!input.startsWith(getPrefix()) || /[\\/:*?"<>|]/.test(input)) {
        Plugins.message.error(
            "文件名必须以 " + getPrefix() + ' 起始，禁止包含 \\ / : * ? " < > |',
        );
...
const checkConfiguration = async () => {
    if (!window.CryptoJS)
        throw "请先安装插件或重新安装插件,或者【右键】=>【重载依赖】...";
...
const reloadDependence = async () => {
    loadDependence();
    Plugins.message.success(`依赖重载成功...`);
};
...
function decrypt(data) {
    try {
        return window.CryptoJS.AES.decrypt(data, Plugin.Secret).toString(
            CryptoJS.enc.Utf8,
        );
    } catch (error) {
        throw "解密失败";
...
class WebDAV {
    ...
    async propfind(url) {
        ...
        const responses = Array.from(xmlDoc.getElementsByTagName("*")).filter(
            (node) => node.tagName.toLowerCase() === "d:response",
        );
        ...
        for (let i = 0; i < responses.length; i++) {
            ...
            const displayname =
                getTextContent(responses[i], "D:displayname") ||
                href.replace(Plugin.DataPath + "/", "");
            list.push({
                href: href,
                displayname: displayname,
                lastModified:
                    getTextContent(responses[i], "D:getlastmodified") || "N/A",
