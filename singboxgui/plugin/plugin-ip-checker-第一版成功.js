/* 节点 IP 检测插件 */
const PATH = 'data/third/plugin-ip-checker'
const FILE_NAME = PATH + '/plugin-ip-checker.json'
const DETECTION_FILE = PATH + '/plugin-ip-checker.json'
...
const onRename = async () => {
    ...
    const text = await Plugins.ReadFile(DETECTION_FILE);
          ...
          <div>
            ...
            <Card style="margin-top:8px">
              <div style="display:flex;gap:8px;flex-wrap:wrap">
                ...
                <Button @click="saveDetectionJsonFile(JSON)">⚡ 保存检测文件</Button>
...
// 获取当前使用的配置（为了获取当前使用的订阅ID）
            ...
            async function saveDetectionJsonFile() {
                ...
                try {
                    await Plugins.WriteFile(DETECTION_FILE, jsonString);
                    Plugins.message.success(`写入【${DETECTION_FILE}】文件成功！`)
                } catch (error) {
                    if (error === '写入失败') {
                        failed = true
                    }
            ...
            return {
                configPath, loadingConfig, configLoadMsg, configLoadOk, loadConfig,
